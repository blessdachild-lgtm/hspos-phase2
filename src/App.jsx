...
const MODULES = [
  {
    ...
    days: [
      { day: 1, title: "Baseline Calibration", phase: "A", objective: "A52 becomes a daily physical habit, not a concept you know about.", content: "Three A52 sessions today: morning, midday, evening.\n\nA52 is the foundational breathing protocol this system runs on: 5-second inhale through the nose, 5-second exhale through the mouth, 2-second hold at the bottom. One cycle is twelve seconds. It activates your parasympathetic nervous system — the biological brake on your stress response. Everything else in this system depends on your ability to run this on demand.\n\nThe only thing you're tracking is whether you did it. Not how it felt. Not whether you're calmer.", logPrompt: "What did your body feel like before and after each session?", metric: "Did you run all three sessions? Yes or no." },
  ...
  },
  {
    ...
    days: [
      ...
      { day: 4, title: "Pace Lock Under Green Signal", phase: "B", objective: "Hold steady when it's working.", content: "Pace Lock: maintaining the same energy, tempo, and investment level you had before a strong signal appeared. Not pulling back. Not escalating. The same pace, held deliberately under the pressure of interest.\n\nEnter an interaction where interest is present. When the Green signal hits, your one job: maintain the same pace, tone, and energy you had before the signal arrived.\n\nNo escalation. No spike. No sudden increase in investment. Hold steady when it's working.", logPrompt: "Did you hold pace? If you spiked, identify the exact moment. What did your body do? What did you say that you wouldn't have said thirty seconds earlier?", metric: "Did you maintain baseline energy when positive signals appeared?" },
  ...
  },
  {
    ...
    days: [
      ...
      { day: 3, title: "The 3-Second Rule — First Real-World Test", phase: "B", objective: "Replace deliberation with pre-loaded triggers.", content: "One rule today: when you see a valid opening, you have three seconds. Not three seconds to decide — three seconds to move.\n\nRun pre-contact A52. Identify your override script when it fires. Then: 3... 2... 1... move.\n\nYou need one rep today. One. The quality of the interaction is irrelevant.", logPrompt: "Did you fire? If not, what script overrode you? If yes — what happened in your body during those three seconds?", metric: "Did you move within the 3-second window at least once?" },
      { day: 4, title: "Satisficing Under Pressure", phase: "B", objective: "Stop optimizing. Start moving.", content: "Satisfice: a decision-making strategy where you act on the first option that meets your minimum criteria rather than scanning for the optimal one. The word combines satisfy and suffice. In this system it means: first valid opening that clears the threshold — move. Not the best opening. Not after three more scans.\n\nTwo reps minimum. You are not allowed to optimize your entry. No waiting for the perfect moment. No scanning for a better target. No rehearsing.\n\nSatisfice — take the first action that clears the minimum threshold:\n1. Opening exists\n2. Engagement is socially appropriate\n3. You're regulated enough to function\n\nIf those three are met, you go.", logPrompt: "Did you satisfice or did you catch yourself optimizing? What did the optimization sound like?", metric: "Did you act on the first valid opening without optimizing?" },
      { day: 5, title: "Multi-Rep Volume Day", phase: "B", objective: "Prove the bottleneck was never ability.", content: "Three reps minimum. Different contexts if available. The 3-second trigger is the default. The override script is identified and rejected on detection.\n\nIf the bottleneck was ability — if you genuinely lacked the skill — doing more reps would expose that. You would fail harder as the volume increased. But the bottleneck isn't ability. It's hesitation. Volume proves that.\n\nYou are only answering one question: Do I move?\n\nThe answer is pre-loaded. If opening exists → move within three seconds.", logPrompt: "Total reps attempted. Total reps fired. Total scripts that tried to run. Action-to-opening ratio: how many times did you move vs. how many openings you saw. Compare that ratio to Day 1.", metric: "Is the action-to-opening ratio improving from Day 1?" },
  ...
  },
  {
    ...
    days: [
      { day: 1, title: "The Inflation Audit", phase: "A", objective: "Accept the overperception bias as neurological fact.", content: "The inflation pattern is a systematic error in signal reading: consistently interpreting social signals as more positive than they actually are. Friendliness read as attraction. Politeness read as interest. A warm interaction read as a green light.\n\nThis is documented neuroscience — not a personal flaw. The brain's social reward system inflates ambiguous signals toward the positive. The inflation pattern is the gap between what you read and what was actually there. This module corrects that gap.\n\nReview your last five social interactions from memory. For each one:\n\n1. What did you read as the signal?\n2. What was the actual outcome?\n\nWhere did your read and reality diverge? That gap is your inflation baseline.", logPrompt: "For each interaction: the read vs. the result. Where did you inflate? Where were you accurate?", metric: "Can you identify your inflation pattern?" },
      ...
      { day: 6, title: "Advance Timing Rep", phase: "C", objective: "Calibrate the transition from conversation to logistics.", content: "Clustered evidence: three or more Green signals present inside a single interaction window. Not one strong signal. Not signals spread across multiple days. Three observable Green behaviors — sustained eye contact, body orientation tracking you, re-engagement after pauses — occurring together in the same interaction.\n\nReal interaction, full stack. When signals cluster — and they must cluster, not just flash — run the Macro-Advance.\n\nMove to specific, low-pressure logistics. Not \"we should hang out sometime.\" A specific invitation with a specific time.\n\nYou advance when mutual investment is present and increasing. Not when you feel ready.", logPrompt: "Did you advance? Was it earned by signal evidence or driven by anxiety? How do you know?", metric: "Did you advance on clustered evidence, not feeling?" },
...
const PRESENCE_MESSAGES = {
  ...
  identity: [
    ...
    "Today is abstinence from validation — no compliments to relieve tension, no smoothing the moment. Warmth comes back later. Right now you're isolating the habit.",
  ...
  decision: [
    ...
    "Today removes doubt. If the bottleneck was ability — if you genuinely lacked the skill — doing more reps would expose that. You would fail harder as volume increased. But the bottleneck isn't ability. It's hesitation. Volume proves that.",
    "All three behavioral systems run today simultaneously — State regulation, Identity anchor, Decision trigger. If any one breaks down, the full loop breaks with it.",
  ...
  calibration: [
    ...
    "One test. One read. Then act or exit. Running a second test isn't persistence — it's forcing an outcome that the signal didn't authorize.",
...
const QUICK_LOG_REASONS = [
  ...
  { id: "avoidance", label: "Chose not to rep today" },
];
...
function GateScreen({ mod, logs, onPass, onRetry }) {
  ...
  const gateConfig = {
    ...
    identity: {
      ...
      signatureHint: "Write the exact statement you built on Day 1. Then describe the moment the Shift occurred — who was in front of you and what changed.",
